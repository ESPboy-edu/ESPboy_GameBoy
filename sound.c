...
extern int32_t maxout;
extern uint8_t divider;
extern int32_t minch1, minch2, minch3, minch4; // Убедись, что minch4 добавлено сюда
...
u8 IRAM_ATTR audio_update() {
    ...
    if (SO.power) {
        // Канал 1
        // ИСПРАВЛЕНО: Было CH2.duty_len.len, заменено на CH1.duty_len.len
        ...
        ch = GENERATE_WAVE(CH1.channel.freq, CH1.envelope.volume, CH1.duty_len.duty);
        ...
        ch = GENERATE_WAVE(CH2.channel.freq, CH2.envelope.volume, CH2.duty_len.duty);
        ...
        ch = (GENERATE_NOISE(CH4.envelope.volume, CH4.NR43 & NR43_DIV_RATIO_BITS)) >> 1;
        // ИСПРАВЛЕНО: Было minch3, заменено на minch4
        if(ch < minch4) minch4 = ch;
        out += ch - minch4;
    }
    ...
    // ИСПРАВЛЕНО: Защита от переполнения (Hard-clipping) для sigmadelta
    // Без этого значения > 255 сбрасываются в 0 и вызывают треск
    if (out > 255) out = 255;
    if (out < 0) out = 0;
